...
select customer_id, count(order_date) attendance
from sales
group by customer_id;
...
case
when exists(select members.customer_id from members where members.customer_id = sales.customer_id and sales.order_date >= members.join_date) then 'Y'
else 'N'
end as member
from sales
